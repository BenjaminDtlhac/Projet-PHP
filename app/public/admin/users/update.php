...
// Récupérer le user grâce à l'id dans l'url GET
require_once '/app/Requests/users.php';

$user = preg_match('/^[0-9]+$/', $_GET['id'] ?? '') ? findOneUserById($_GET['id']) : null;

if (!$user) {
    $_SESSION['messages']['danger'] = 'User ontrouvable';
...
if (
    ...
    // Nettoyage de données pour faille XSS
    ...
    if (!$changeEmail || !findOneUserByEmail($user['email'])) {
        // On peut modifier l'utilisateur
        if (updateUser($user['id'], $firstName, $lastName, $email, $password)) {// User modifié sans erreur
            // On a définit un message de succès
            $_SESSION['messages']['success'] = "User modifié avec succès";

            // On redirige l'utlisateur vers la liste des users
            header("Location: /admin/users");
            exit(302);
        } else {
